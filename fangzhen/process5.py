...
def make_wrapper(func):
    def wrapper(*args, **kwargs):
        ...
        for i, item in enumerate(args_list):
            if i in [1, 3]:
                new_args_one.append(item+41)  # +58是自动化  +50标签栏  -41firefox +17benren
            else:
                new_args_one.append(item)
        # 求相对比率
        rate_list = []
        for i, item in enumerate(new_args_one):
            if isinstance(item, int):
                if i % 2 == 0:
                    rate_list.append((item-left_value)/(right_value-left_value))
                if i % 2 == 1:
                    rate_list.append((item-top_value)/(buttom_value-top_value))
            else:
...
def resource_configuration_module():
    ...
    time.sleep(5)
    # todo:坐标待测试
    save_picture(1083, 437, 1179, 457, os.getcwd(), '资源配置')
    time.sleep(5)
...
def business_design_module():
    ...
    time.sleep(5)
    save_picture(631, 375, 773, 680, os.getcwd(), '产品流程')
    time.sleep(2)
